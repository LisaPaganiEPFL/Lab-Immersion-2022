...
@njit
def cycle(in_numbers, migration_matrix, fitnesses, nb_cycles, growth_factor, Nmin, start_follow_numbers, size_follow_numbers, start_follow_k, start_cycle, print_frequency, save_dynamics=False, dilution_std=None, fitness_range=None, structure = None):
    N_demes, N_types= np.shape(in_numbers)
    s = fitnesses[0] - 1
    ...
    if start_follow_numbers is None:
        ...
        follow_k = np.zeros((size_follow_numbers,N_demes), dtype=np.int64)
    else:
        follow_numbers=start_follow_numbers.copy()
        follow_k = start_follow_k.copy()
    ...
    for i in range(nb_cycles):
        ...
        if fitness_range is None:  
            ...
            if structure == "star":
                ...
                fit = np.random.uniform(1+s-fitness_range,1+s+fitness_range,1)[0]
                fitness = np.array([fit,1.])
                numbers1=growth_event(numbers,fitness,growth_factor)
        ...
        if (start_cycle+i)%print_frequency==0 and ((i+start_cycle)/print_frequency)<size_follow_numbers:
            ...
            follow_k[int(i+start_cycle),:]=Nmin_table
    ...
    if keep_going:
        follow_numbers, follow_k, end_cycle, fixation= cycle(numbers, migration_matrix, fitnesses, nb_cycles, growth_factor, Nmin, follow_numbers, size_follow_numbers, follow_k, start_cycle+end_cycle, print_frequency, save_dynamics, dilution_std, fitness_range, structure)
    return follow_numbers, follow_k, end_cycle, fixation
...
def fixation_probability(in_numbers, folder, migration_matrix, fitnesses, nb_sim, nb_cycles, growth_factor, Nmin, size_follow_numbers=10000, print_frequency=1, save_dynamics=False, dilution_std=None, fitness_range=None, structure = None):
    ...
    for i in range(nb_sim):
        ...
        start_follow_k=None
        follow_numbers,follow_k, end_cycle, fixation = cycle(in_numbers, migration_matrix, fitnesses, nb_cycles, growth_factor, Nmin, start_follow_numbers, size_follow_numbers, start_follow_k, start_cycle, print_frequency, save_dynamics, dilution_std, fitness_range, structure)
